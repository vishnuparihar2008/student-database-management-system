...
const Sidebar = () => {
    ...
    const navItems = [
        ...
        {name: "Marksheet", href: "/marksheet", icon: FileQuestionMark},
            ...
            <div className="px-5 pt-6 pb-6 border-b border-white/6">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="font-semibold text-[13px] text-white tracking-wide">Student MS</p>
                ...
                { navItems.map((item) => {
                    ...
                    return (
                        <Link key={item.name} to={item.href} className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? "bg-indigo-500/12 text-indigo-300" : "text-slate-300 hover:text-white hover:bg-white/4"}`}>
                            ...
                            {isActive && <ChevronRightIcon className="w-3.5 -3.5 text-indigo-500/50" />}
                        </Link>
                    )
                })}
            </div>
